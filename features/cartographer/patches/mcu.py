...
class MCU_trsync:
    ...
    def _build_config(self):
        ...
        set_timeout_tag = mcu.lookup_command_tag(
            "trsync_set_timeout oid=%c clock=%u") & 0xffffffff
        trigger_tag = mcu.lookup_command_tag("trsync_trigger oid=%c reason=%c") & 0xffffffff
        ...
        self._trdispatch_mcu = ffi_main.gc(ffi_lib.trdispatch_mcu_alloc(
            ...
            self._cmd_queue, self._oid & 0xffffffff, set_timeout_tag, trigger_tag,
...
class TriggerDispatch:
    ...
    def stop(self):
        ...
        # Use exact equality to handle K2's reason code ordering
        # Only flag actual comm timeouts, not PAST_END_TIME or HOST_REQUEST
        if any([r == MCU_trsync.REASON_COMMS_TIMEOUT for r in res]):
            raise self._mcu.get_printer().command_error(
                """{"code":"key401", "msg":"Communication timeout during homing", "values":[]}""")
        return res[0]
...
class MCU_endstop:
    ...
    def home_wait(self, home_end_time):
        ...
        # TriggerDispatch.stop() now raises exception for actual timeouts
        # so we only need to check for successful endstop hit
...
class MCU:
    ...
    def __init__(self, config, clocksync):
        ...
        self._reconnecting = False  # True during reconnection attempts
        self.reconnect_interval = config.getfloat('reconnect_interval', 2.0) + 0.12
        self._non_critical_reconnect_event_name = "non_critical_mcu_%s:reconnected" % (self._name,)
        self._non_critical_disconnect_event_name = "non_critical_mcu_%s:disconnected" % (self._name,)
    ...
    def recon_mcu(self):
        self._reconnecting = True  # Allow serial commands during reconnection
        try:
            ...
            self._connect()
            self._gcode.respond_info("mcu: '%s' reconnected, loading models..." % (self._name,))
            self._printer.send_event(self._non_critical_reconnect_event_name)
            return True
        except Exception as e:
            logging.exception("Non-critical MCU '%s' reconnect failed: %s", self._name, str(e))
            self.non_critical_disconnected = True
            self._get_status_info['non_critical_disconnected'] = True
            self._clocksync.disconnect()
            self._disconnect()
