...
class PrinterTemperatureMCU:
    def __init__(self, config):
        ...
        self.printer.register_event_handler("klippy:mcu_identify",
                                            self._mcu_identify)
        # Register for non-critical MCU reconnect to complete init if skipped
        ...
        if hasattr(mcu, 'is_non_critical') and mcu.is_non_critical:
            ...
            self.printer.register_event_handler(reconnect_event, self._mcu_identify_on_reconnect)
    ...
    def _mcu_identify(self):
        ...
        mcu = self.mcu_adc.get_mcu()
        # Skip initialization if this is a non-critical MCU that is disconnected
        if hasattr(mcu, 'is_non_critical') and mcu.is_non_critical:
            if hasattr(mcu, 'non_critical_disconnected') and mcu.non_critical_disconnected:
                logging.info("temperature_mcu: Skipping %s - non-critical MCU disconnected", 
                             mcu.get_name())
                return
